...
import {
  X, MapPin, Ship, ArrowRight, CheckCircle2, Anchor,
} from "lucide-react";
...
export default function ShipServiceDetail({ service, onClose }) {
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      toast("Request sent! Opening chat thread.", "success");
      navigate("/home", {
        state: {
          serviceOrder: {
            shipName: service.ship_name,
            shipInitials: init(service.ship_name),
            services: selected,
      ...
      setError(err.message || "Failed to place request.");
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-end p-3 sm:p-4">
      ...
      <motion.div
        ...
        role="dialog" aria-modal="true" aria-label={`${service.ship_name} details`}
      ...
      >
        ...
        <div className="shrink-0 border-b border-white/[0.04] px-5 py-5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-gradient-to-br from-white/10 to-white/[0.03] text-sm font-bold text-white shadow-sm">
                {init(service.ship_name)}
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <Ship className="h-3 w-3 text-white/30" />
                  <span className="text-[8px] uppercase tracking-[0.18em] text-white/25">Vessel</span>
                </div>
                <h2 className="truncate text-sm font-semibold text-white mt-0.5">{service.ship_name}</h2>
              </div>
            </div>
            <button onClick={loading ? undefined : onClose} disabled={loading}
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-white/[0.06] bg-white/[0.03] text-white/40 hover:text-white hover:bg-white/[0.06] transition-all duration-200 disabled:opacity-40"
              aria-label="Close">
          ...
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-xl border border-white/[0.04] bg-white/[0.02] px-3 py-1.5">
            ...
            <span className="text-[11px] text-white/45 truncate">{service.current_location}</span>
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5">
          ...
          <div className="space-y-1.5" role="group" aria-label="Service selection">
            {service.services_offered.map((id) => {
              const on = selected.includes(id);
              return (
                <motion.button key={id} type="button" whileTap={{ scale: 0.98 }} onClick={() => { if (loading) return; setSelected((p) => p.includes(id) ? p.filter((s) => s !== id) : [...p, id]); setError(null); }}
                  aria-pressed={on}
          ...
          {service.services_offered.length === 0 && (
            <p className="text-xs text-white/25 text-center py-8">No services currently listed for this vessel.</p>
          )}
        </div>

        {/* Footer */}
        <div className="shrink-0 border-t border-white/[0.04] px-5 py-4">
          {error && <p className="text-[11px] text-red-400 mb-2" role="alert">{error}</p>}
          {selected.length > 0 && (
            <p className="text-[11px] text-white/35 mb-2 truncate">
              Requesting: <span className="text-cyan-300">{selected.map((id) => SVC[id]?.label || id).join(", ")}</span>
            </p>
          )}
          <Button onClick={handleOrder} disabled={!selected.length || loading} loading={loading} className="w-full gap-2 text-sm font-medium" size="lg">
            {loading ? "Sending Request" : <><ArrowRight className="h-4 w-4" /> Request &amp; Chat</>}
          </Button>

          {!selected.length && !loading && (
            <p className="text-[9px] text-white/15 text-center mt-2">Select a service to continue</p>
          )}
        </div>
      </motion.div>
    </div>
