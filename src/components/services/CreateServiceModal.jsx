...
export default function CreateServiceModal({ onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim()) return setError("Ship name required.");
    if (!location.trim()) return setError("Location required.");
    if (!selected.length) return setError("Select at least one service.");

    setLoading(true); setError(null);
    try {
      const result = await createShipService({ ship_name: name.trim(), current_location: location.trim(), services_offered: selected });
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.98 }}
        ...
        <div className="flex items-center justify-between border-b border-white/[0.04] px-5 py-4">
          ...
          <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/[0.06] bg-white/[0.03] text-white/40 hover:text-white transition-all duration-200">
            <X className="h-3.5 w-3.5" />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          {error && <div className="rounded-xl border border-red-500/15 bg-red-500/6 px-3.5 py-2.5"><p className="text-[11px] text-red-400">{error}</p></div>}
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" value={name} onChange={(e) => { setName(e.target.value); setError(null); }}
              ...
              className="w-full h-10 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3.5 text-sm text-white placeholder:text-white/20 outline-none transition-all duration-300 focus:border-white/18 focus:bg-white/[0.04] disabled:opacity-40" />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" value={location} onChange={(e) => { setLocation(e.target.value); setError(null); }}
          ...
          <div className="flex gap-2 pt-1">
            <Button type="button" variant="ghost" className="flex-1 h-10 text-xs" onClick={onClose} disabled={loading}>Cancel</Button>
